...
An example of lambda abstraction : $\lambda x . \lambda y . y x$. We try to avoid using brackets in lambda abstractions
...
```mermaid
graph TD;
    lx["$\lambda x . x (\lambda y . y)$"]

    lx_head["$\lambda x$"]
    lx_body["$x (\lambda y . y)$"]
    
    lx_body_1["$x$"]
    lx_body_2["$(\lambda y . y)$"]
    ...
    lx_body_2 -- Head --> ly_head
    lx_body_2 -- Body --> ly_body
```
